...
def seed_demo():
    ...
    try:
        ...
        if not bs_2025:
            ...
            db.commit()
            print(f"   ✅ Seeded 2025 NAS / NSBU balance sheet (SUBMITTED, 95M ₽ balanced)")
        else:
            print("   ℹ️ 2025 Balance sheet data already exists")
        ...
        if tmpl_count == 0:
            for t_name, t_desc, t_type, t_cc, t_taxes, t_cfg in templates_data:
                ...
                db.add(tmpl)
            db.commit()
            print("   ✅ Seeded 4 industry-standard report templates with smart configuration")
        else:
            # Backfill configurations for existing templates if missing
            updated_any = False
            for t_name, t_desc, t_type, t_cc, t_taxes, t_cfg in templates_data:
            ...
            if updated_any:
                db.commit()
                print("   ✅ Backfilled smart configuration for existing report templates")
            else:
        ...
        if reports_count == 0:
            ...
            analysis2 = ReportAnalysis(
                ...
                warnings=0,
                errors=0,
                summary="VAT calculation verified successfully. Full compliance.",
